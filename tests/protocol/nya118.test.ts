...
import { createNya118Codec } from '../../src/protocol/nya118';
...
describe('nya118 codec', () => {
  it('round-trips mixed unicode text without normalization', async () => {
    const codec = createNya118Codec(createCompressionAdapter());
    const text = '喵！Hello～〜!😺\n第二行';
  ...
  it('uses a one-byte codec frame for short raw text', async () => {
    const codec = createNya118Codec(createRawCompressionAdapter());
