...
const EXPECTED_SPACE_SUFFIX_BASE_TOKENS = [
  '！',
  '～',
  '哈',
  '—',
...
describe('nya256 token table', () => {
  ...
  it('keeps no-separator decoding reversible with longest-match tokens', () => {
    ...
    const bangIndex = TOKEN_TABLE.indexOf('！');
    const bangSpaceIndex = TOKEN_TABLE.indexOf('！ ');
    const newlineSpaceIndex = TOKEN_TABLE.indexOf('\n ');
    const digits = [blackCatIndex, catIndex, blackCatIndex, bangSpaceIndex, bangIndex, newlineSpaceIndex];
    ...
    expect(bangSpaceIndex).toBeGreaterThanOrEqual(0);
    expect(newlineSpaceIndex).toBeGreaterThanOrEqual(0);
    expect(decodeCatToDigits(encodeDigitsToCat(digits))).toEqual(digits);
    expect(decodeCatToDigits('！ ')).toEqual([bangSpaceIndex]);
  });

  it('round-trips mixed punctuation, CJK, and ASCII tokens', () => {
    const digits = [0, 1, 2, 9, 10, 11, 12, 58, 80, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 170, 142, 143, 150];
    const cat = encodeDigitsToCat(digits);
  ...
  it('exposes the expected 256-token values and ordering', () => {
    ...
    expect(TOKEN_TABLE.slice(82, 142)).toEqual([
    ...
    expect(TOKEN_TABLE.slice(142, 151)).toEqual(EXPECTED_FACE_TOKENS);
    expect(TOKEN_TABLE[150]).toBe('(•ᆺ•)');
    expect(TOKEN_TABLE.slice(151, 154)).toEqual(['\n', ';', '；']);
    expect(TOKEN_TABLE.slice(154, 170)).toEqual(EXPECTED_CAT_EMOJI_TOKENS);
    expect(TOKEN_TABLE.slice(170)).toEqual(
      EXPECTED_SPACE_SUFFIX_BASE_TOKENS.map((token) => `${token} `),
    );
    expect(TOKEN_TABLE).not.toContain('(=^.^=)');
    expect(TOKEN_TABLE).not.toContain('（=^.^=）');
  });
  ...
  it('rejects unknown prefixes', () => {
    ...
    expect(() => decodeCatToDigits(' ')).toThrowError(/token/i);
  });
});
