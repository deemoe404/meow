...
import { createNya119Codec } from '../../src/protocol/nya119';
...
describe('nya119 codec', () => {
  it('round-trips mixed unicode text without normalization', async () => {
    const codec = createNya119Codec(createCompressionAdapter());
    const text = '喵！Hello～〜!😺\n第二行';
  ...
  it('uses a one-byte codec frame for short raw text', async () => {
    const codec = createNya119Codec(createRawCompressionAdapter());
