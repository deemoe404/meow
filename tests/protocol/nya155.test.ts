...
import { createNya155Codec } from '../../src/protocol/nya155';
...
describe('nya155 codec', () => {
  it('round-trips mixed unicode text without normalization', async () => {
    const codec = createNya155Codec(createCompressionAdapter());
    const text = '喵！Hello～〜!😺\n第二行';
  ...
  it('reports token counts from base155 digits for fixed raw samples', async () => {
    const codec = createNya155Codec(createRawCompressionAdapter());
    const shortText = 'short';
    const longText = '在一个理想的猫语翻译器里，协议层和外观层必须明确分开。协议层负责把任意 Unicode 文本安全地转成 UTF-8 字节，再经过 raw 或 zstd-dict 选择、frame 打包、base155 digit 切片和固定 token 表映射，最后得到一串可以公开传播的猫语。';
