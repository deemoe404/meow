...
一个部署到 GitHub Pages 的静态单页网站，实现可逆猫语短码协议。

## 特性

- 任意 Unicode 文本 `-> UTF-8 -> codec frame -> base4096 digit -> 猫语大词表`
- `raw` 与 `zstd-dict` 双 codec，编码时自动选择更短 payload
- 输出使用 4096-token 变长猫语词表：2048 个双字猫叫 + 1792 个中文猫叫三连音 + 256 个 `meo` / `mow` / `mia` / `aoo` / `aou` 等英文猫叫，不再带格式魔数
- 浏览器本地编解码，无服务端依赖
...
当前编码保持原始 Unicode，不做 NFC / NFKC 归一化，也不折叠全角 `！`、全角 `～`、全角 `；` 与 `〜`。
...
5. 把每个 digit 映射成 prefix-free 猫语 token，例如 `喵喵`、`咪咕`、`咿呼噜`、`meo`、`mow`、`mia`、`aoo`、`aou`

这个选择来自信息论约束：压缩后的 payload 近似均匀随机，类似密码学里的密文，高频字节已经基本不存在，因此继续对 payload 做 Huffman 式频率编码收益很小。旧版 256-token 表每个 token 只承载 8 bit，但平均需要约 2.49 个可见字符；新版每个猫语 token 承载 12 bit，平均占 2.5 个可见字符。对 `n` 字节 frame，新版期望长度约为 `2.5 * ceil(8n / 12)`，约为 `1.67n`，同时恢复初版里中英文猫叫混合的猫语观感。
