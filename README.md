...
一个部署到 GitHub Pages 的静态单页网站，实现可逆猫语协议 `nya155-zh11`，并支持可选 8018-token 词表。

## 特性

- 任意 Unicode 文本 `-> UTF-8 -> codec frame -> baseN digit -> 猫语 token`
- `raw` 与 `zstd-dict` 双 codec，编码时自动选择更短 payload
- 默认 155-token 词表，也可在页面右上角切换到 8018-token 扩展词表
- 浏览器本地编解码，无服务端依赖
...
4. codec frame 走无 padding 的 N 进制 digit 切片：默认词表使用 base155，扩展词表使用 base8018
...
扩展 8018-token 词表不会替换默认词表；它只在页面右上角开关打开时使用。生成规则是：

1. 从默认词表移除 5 个单个 ASCII 标点 token：`,`、`!`、`~`、`?`、`;`，并移除独立空格 token
2. 保留默认词表里其余 token
3. 用 `!` 和 `?` 生成长度 `1..4` 的全部有序后缀，共 `2 + 4 + 8 + 16 = 30` 种
4. 对默认词表中的 57 个纯汉字 token 和 72 个纯英文字母 token，各追加上述 30 种后缀，新增 `129 * 30 = 3870` 个 token
5. 对上述 129 个纯汉字/英文字母 token 追加空格，新增 `129` 个 token；再对 3870 个标点后缀 token 追加空格，新增 `3870` 个 token

因此扩展词表总数为 `155 - 5 - 1 + 3870 + 129 + 3870 = 8018`。扩展词表中 `mew!?`、`mew!?!? ` 这类 token 会和 `mew` 形成前缀关系，解码器用最长匹配规则解析。
